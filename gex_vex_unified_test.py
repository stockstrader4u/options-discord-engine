r"""
gex_vex_unified_test.py -- TEST SCRIPT for the new consolidated
single-card GEX/VEX dashboard (the "v9" design), posted to a TEST
webhook only.

WHY THIS EXISTS: the production pipeline (gex_vex_combined_daily.py)
currently posts 7 separate Mag 7 cards + 1 separate SPY/QQQ/IWM
dashboard -- 8 images and 8+ Discord messages every day, confirmed by
the user to be too much to scroll through and digest. This script
prototypes a SINGLE consolidated image (SPY/QQQ/IWM gradient-bar
cards + a Mag 7 positioning table + a "Today's Focus" callout panel)
built and iterated on over several rounds directly against a reference
mockup the user provided, before it gets merged into production.

WHAT HAPPENS TO THE "SINCE YESTERDAY" DAY-OVER-DAY COMPARISON:
Nothing changes about it. That feature lives entirely in Discord TEXT
(gex_vex_history.save_and_compare(), posted as embed fields / plain
text), completely independent of what the rendered card image looks
like. This script proves that by actually calling the same history
module and posting its real output alongside the new image -- not
just asserting it still works.

DATA: real, live numbers from gex_vex.compute_gex_vex() for all 10
tickers (Mag 7 + SPY/QQQ/IWM) -- the same function the production
pipeline uses, unmodified. gex_vex.py and gex_vex_history.py are NOT
touched by this script.

SAFETY: posts ONLY to GEX_UNIFIED_TEST_DISCORD_WEBHOOK (a NEW,
separate test webhook -- set this to a test channel, not the real GEX
channel). Does not touch gex_vex_combined_daily.py or the production
GEX_DISCORD_WEBHOOK. Run this for a few days to review before merging
the new card design into production.
...
TEST_WEBHOOK = os.environ["GEX_UNIFIED_TEST_DISCORD_WEBHOOK"]
...
def pick_todays_focus(core_results, mag7_results):
    ...
    with_em = [r for r in all_valid if r.get("expected_move")]
    if with_em:
...
def render_unified_card(core_results, mag7_results, focus_items, week_label, out_path):
    FIG_W = 15.5
    ...
    for i, t in enumerate(valid_core):
        ...
        bw = 1.75
        pill(ax, cx + card_w - bw - 0.28, cy + 0.30, bw, 0.40, badge_color, badge_txt, fontsize=8.5)
...
def main():
    print("=== UNIFIED GEX/VEX CARD TEST -- posting to TEST webhook only ===\n")
    ...
    print("\nRunning Since Yesterday comparison (real gex_vex_history, unchanged)...")
    ...
    focus_items = pick_todays_focus(core_results, mag7_results)
    for t, color, tag, desc in focus_items:
        print(f"  {t['ticker']}: {tag}")

    print("\nRendering unified card...")
    out_path = "gex_unified_test.png"
    render_unified_card(core_results, mag7_results, focus_items, week_label, out_path)
    print(f"  saved to {out_path}")

    print("\nPosting to test webhook...")
    header = f"\U0001F4CA **DAILY GEX / VEX DASHBOARD TEST \u2014 {week_label}**"
    post_text_to_discord(header)

    if since_yesterday_lines:
        post_text_to_discord("**\U0001F4C5 Since Yesterday**\n" + "\n".join(since_yesterday_lines))
    else:
        post_text_to_discord("_No prior-day snapshot yet -- Since Yesterday will populate starting tomorrow._")
