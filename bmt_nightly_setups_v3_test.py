...
memory cost. The always-on APScheduler process held RAM 24/7 to fire
one ~minutes-long job per day. With RUN_MODE=once this file instead runs
the job a single time and exits, so Railway only bills for the minutes
it actually runs. NOTHING about the job itself changed -- main() is
byte-for-byte the same function.
  ...
  * RUN_MODE=once: intended for a Railway Cron Schedule. Railway cron
    ...
    in about a second. Runs therefore stay at 6pm ET through every DST
    change with no manual edits twice a year.
  * FORCE_PUBLISH_V3=1 still wins over everything (manual test runs).
  * An exception inside the job is logged and swallowed (exit code 0)
    ...
    and double-post partway through a partial failure.
"""
...
def run_once_from_cron():
    ...
    try:
        ...
        # re-posting already-published setups.
        print("\u2717 RUN_MODE=once: v3 nightly job raised an exception (traceback below); NOT retrying.")
        traceback.print_exc()
